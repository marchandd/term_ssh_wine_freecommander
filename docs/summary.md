...
Return to [index](https://github.com/marchandd/term_ssh_wine_freecommander/ "Index")
...
- Run one same `freecommander.sh` command in cyclic mode to run sub programs:  
One run to set Wine emulator, one run to install and one run to launch.  
- freecommander alias is not available at the end of progam. You have to make alias activation later.
...
:computer: `docker run -d -p XXX.XXX.XXX.XXX:YYYYY:22 --name latest_term_ssh_wine_freecommander -v LOCALPATH:/data marchandd/term_ssh_wine_freecommander`
...
- :computer: `freecommander.sh` to set Wine emulator.  
- :computer: `freecommander.sh` to install programm.  
- :computer: `freecommander.sh` to launch programm.  
